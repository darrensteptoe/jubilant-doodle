...
- District bridge reduction: v3 District standard controls now hydrate bridge-first from `view.form` (values, select options, disabled map) for race context, electorate, ballot topline, turnout baseline, and electorate structure, with legacy value/disabled sync retained only as a no-bridge fallback.
- District bridge reduction: v3 District standard writes now route through runtime District bridge methods (`setFormField`, `addCandidate`) instead of direct hidden-control proxies; candidate table row editing and user-defined split rows remain legacy-mirrored in this phase.
