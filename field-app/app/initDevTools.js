export function initDevToolsModule(ctx){
  ...
  const renderResult = (r) => {
    ...
    if (typeof setSelfTestGateStatus === "function"){
      setSelfTestGateStatus(engine.selfTest.gateFromSelfTestResult(r));
      updateSelfTestGateBadge();
    }
    ...
    let lastGood = null;
    try{
      lastGood = safeJsonParse(localStorage.getItem(FPE_LAST_GOOD_KEY) || "");
    } catch {}
  ...
  const buildCurrentMcContext = () => {
    ...
    const modelInput = {
      ...
      candidates: (Array.isArray(state.candidates) ? state.candidates : []).map((c) => ({ id: c.id, name: c.name, supportPct: safeNum(c.supportPct) })),
