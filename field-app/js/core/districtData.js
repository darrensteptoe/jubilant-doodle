...
function normalizeCrosswalk(row){
  ...
  return {
    ...
    source: toIdOrNull(row.source),
    rowsUrl: toIdOrNull(row.rowsUrl ?? row.rows_url ?? row.dataUrl ?? row.data_url),
    refreshedAt: toIsoOrNull(row.refreshedAt),
    hash: toIdOrNull(row.hash),
    isLatest: !!row.isLatest,
  };
...
function normalizeCatalogDataset(row, kind){
  ...
  return {
    ...
    label: toCleanString(row.label),
    source: toIdOrNull(row.source),
    manifestUrl: toIdOrNull(row.manifestUrl ?? row.manifest_url ?? row.sourceManifestUrl ?? row.source_manifest_url),
    rowsUrl: toIdOrNull(row.rowsUrl ?? row.rows_url ?? row.dataUrl ?? row.data_url ?? row.sourceRowsUrl ?? row.source_rows_url),
    vintage: toIdOrNull(row.vintage),
