...
import { resolveOfficeContextDisplayLabel } from "../core/officeContextLabels.js";
...
function raceTypeDisplayLabel(state){
  return resolveOfficeContextDisplayLabel({
    appliedTemplateId: state?.templateMeta?.appliedTemplateId,
...
export function generateCalibrationSourceBrief(state){
  ...
  const now = nowIso();
  const raceTypeLabel = raceTypeDisplayLabel(state);
...
export function generateScenarioSummaryBrief(state){
  ...
  const lines = [
    ...
    `Race type: ${raceTypeDisplayLabel(state)}`,
    `Template benchmark scope: ${cleanString(benchmarkScope?.benchmarkKey) || "default"}`,
