...
export function runSelfTests(engine){
  ...
  // --- CORE MATH AUDIT (identity + sensitivity) ---
  test("CoreMath: identity checks (turnoutVotes, early split, votesPer1pct, persuasionUniverse)", () => {
    const fx = FIXTURES.find(f => f.id === "baseline_small") || FIXTURES[0];
    assert(fx && fx.spec && fx.spec.modelInput, "Missing fixture modelInput");
    const input = structuredClone(fx.spec.modelInput);
    const res = engine.computeAll(input);
    assert(res && res.turnout && res.expected, "computeAll() missing turnout/expected");
    const U = input.universeSize;
    const expPct = res.turnout.expectedPct;
    assert(isFiniteNum(U) && U > 0, "Universe size invalid");
    assert(isFiniteNum(expPct), "Expected turnout pct invalid");

    const expectedTurnoutVotes = Math.round(U * (expPct / 100));
    assert(res.expected.turnoutVotes === expectedTurnoutVotes, `turnoutVotes mismatch: got ${res.expected.turnoutVotes}, expected ${expectedTurnoutVotes}`);

    const earlyVotes = res.expected.earlyVotes;
    const edVotes = res.expected.edVotes;
    assert(isFiniteNum(earlyVotes) && isFiniteNum(edVotes), "Early/ED votes invalid");
    assert(earlyVotes + edVotes === res.expected.turnoutVotes, "earlyVotes + edVotes must equal turnoutVotes");

    const v1 = res.turnout.votesPer1pct;
    const v1exp = U * 0.01;
    assert(approx(v1, v1exp, 1e-9), `votesPer1pct mismatch: got ${v1}, expected ${v1exp}`);

    const pPct = input.persuasionPct;
    if (pPct != null){
      const pU = res.expected.persuasionUniverse;
      const p = Math.max(0, Math.min(100, Number(pPct)));
      const pUexp = Math.round(U * (p / 100));
      assert(pU === pUexp, `persuasionUniverse mismatch: got ${pU}, expected ${pUexp}`);
    }
  });

  test("CoreMath: earlyVoteExp only splits turnoutVotes (does not change turnout total)", () => {
    const fx = FIXTURES.find(f => f.id === "baseline_small") || FIXTURES[0];
    const base = structuredClone(fx.spec.modelInput);
    const baseRes = engine.computeAll(base);
    const baseTurnoutVotes = baseRes?.expected?.turnoutVotes;
    assert(isFiniteNum(baseTurnoutVotes), "Baseline turnoutVotes invalid");

    const hi = structuredClone(base);
    hi.earlyVoteExp = 80;
    const hiRes = engine.computeAll(hi);
    assert(hiRes?.expected?.turnoutVotes === baseTurnoutVotes, "turnoutVotes changed when only earlyVoteExp changed");
    assert(hiRes.expected.earlyVotes !== baseRes.expected.earlyVotes, "earlyVotes did not change when earlyVoteExp changed");
    assert(hiRes.expected.earlyVotes + hiRes.expected.edVotes === baseTurnoutVotes, "Split invalid after earlyVoteExp change");

    const lo = structuredClone(base);
    lo.earlyVoteExp = 0;
    const loRes = engine.computeAll(lo);
    assert(loRes?.expected?.turnoutVotes === baseTurnoutVotes, "turnoutVotes changed when only earlyVoteExp changed (low)");
    assert(loRes.expected.earlyVotes === 0, "earlyVotes should be 0 when earlyVoteExp=0");
    assert(loRes.expected.edVotes === baseTurnoutVotes, "edVotes should equal turnoutVotes when earlyVoteExp=0");
  });

  test("CoreMath: scale invariance sanity (universe +10% => turnoutVotes/persuasionUniverse ~+10%)", () => {
    const fx = FIXTURES.find(f => f.id === "baseline_small") || FIXTURES[0];
    const base = structuredClone(fx.spec.modelInput);
    const r0 = engine.computeAll(base);
    assert(isFiniteNum(r0?.expected?.turnoutVotes), "Baseline turnoutVotes invalid");
    assert(isFiniteNum(r0?.expected?.persuasionUniverse), "Baseline persuasionUniverse invalid");

    const up = structuredClone(base);
    up.universeSize = Math.round(base.universeSize * 1.10);
    const r1 = engine.computeAll(up);
    const scale = up.universeSize / base.universeSize;

    const t0 = r0.expected.turnoutVotes;
    const t1 = r1.expected.turnoutVotes;
    const p0 = r0.expected.persuasionUniverse;
    const p1 = r1.expected.persuasionUniverse;

    assert(isFiniteNum(t1) && isFiniteNum(p1), "Scaled outputs invalid");
    assert(Math.abs((t1 / t0) - scale) <= 0.02, `turnoutVotes scale drift: ${(t1/t0).toFixed(3)} vs ${scale.toFixed(3)}`);
    assert(Math.abs((p1 / p0) - scale) <= 0.02, `persuasionUniverse scale drift: ${(p1/p0).toFixed(3)} vs ${scale.toFixed(3)}`);
  });
