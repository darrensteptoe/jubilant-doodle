...
export function wireIntelChecksEvents(ctx){
  ...
  if (els.btnIntelBenchmarkLoadDefaults){
    els.btnIntelBenchmarkLoadDefaults.addEventListener("click", () => {
      ...
      setBenchmarkStatus(
        `Loaded defaults for '${result.benchmarkKey || formatOfficeContextLabel(result.raceType, { legacyIntent: result.benchmarkKey })}' (${result.created} new, ${result.updated} updated).`,
        "ok"
      );
      commitUIUpdate();
    });
