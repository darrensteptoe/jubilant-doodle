...
const API_PROXY_EDGE_BLOCK_ERROR = "API proxy request was blocked before reaching Worker (HTTP 403). Check Cloudflare Access/WAF rules for /api/*.";
  ...
  if (!response?.ok){
    ...
    if (isJson){
      try{
        ...
        return {
          ok: false,
          code: String(payload?.code || "weather_proxy_error"),
          message: String(payload?.message || `Weather proxy request failed (${status || "unknown"}).`),
        };
      } catch {}
    }
    if (status === 403 && !readHeader(response, PROXY_MARKER_HEADER)){
      return {
        ok: false,
        code: "CLOUDFLARE_BLOCKED",
        message: API_PROXY_EDGE_BLOCK_ERROR,
      };
    }
    return {
      ok: false,
      code: "weather_proxy_failed",
      message: `Weather proxy request failed (${status || "unknown"}).`,
    };
  }
